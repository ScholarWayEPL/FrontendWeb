...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, allowedRoles }) => {
  ...
  if (user?.role === 'ROLE_ADMIN_ETABLISSEMENT' && (location.pathname === '/' || location.pathname === '/dashboard')) {
    return <Navigate to="/etablissement/dashboard" replace />;
  }

  // Redirection automatique pour super_admin qui accède aux routes admin établissement (pas /etablissements)
  if (user?.role === 'ROLE_ADMINISTRATEUR' && location.pathname.startsWith('/etablissement/')) {
    return <Navigate to="/dashboard" replace />;
  }

  // Redirection automatique pour admin_etablissement qui accède aux routes super admin
  if (user?.role === 'ROLE_ADMIN_ETABLISSEMENT' && !location.pathname.startsWith('/etablissement') && location.pathname !== '/parametres') {
    return <Navigate to="/etablissement/dashboard" replace />;
  }
